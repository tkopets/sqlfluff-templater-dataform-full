...
- **`parsing_method`**: This templater offers three methods for parsing `.sqlx` files before compilation. "Parsing" here means identifying the different blocks like `config {...}`, `js {...}`, and `${...}`.
  - **`lexer` (Default)**: A context-tracking lexer that understands SQL and JS strings, template literals and comments. Because it tracks context, a brace, `${` or comment marker that appears *inside* a string or comment cannot mis-terminate a block (e.g. `js { const x = "}"; }` or a `${...}` inside a SQL comment are handled correctly). Recommended for all use cases.
  - **`regex`**: A fast parser that uses regular expressions. Lighter-weight, but can mis-slice blocks that contain braces or `${` inside strings/comments, or deeply nested braces.
  - **`char`**: A brace-counting character parser. Like `regex`, it is not string/comment-aware.
